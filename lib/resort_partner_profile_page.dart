...
class ResortPartnerProfilePage extends StatefulWidget {
  const ResortPartnerProfilePage({super.key});

  @override
  State<ResortPartnerProfilePage> createState() =>
      _ResortPartnerProfilePageState();
}

class _ResortPartnerProfilePageState
    extends State<ResortPartnerProfilePage> {
  ...
  double? _longitude;
  bool _exists = false;
  bool _resortApproved = false;
  bool _resortActive = false;
  String _resortStatus = 'not_created';
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      _city.text =
          resort['city']?.toString() ?? '';
      _area.text =
          resort['area']?.toString() ?? '';
      ...
      _cancellationPolicy.text =
          resort['cancellationPolicy']
                  ?.toString() ??
              '';

      _houseRules.text =
          resort['houseRules']?.toString() ?? '';
      ...
      _resortApproved =
          resort['isApproved'] == true;
      _resortActive =
          resort['isActive'] == true;
      _resortStatus =
          resort['status']?.toString().trim() ??
              (_exists
                  ? 'pending'
                  : 'not_created');
  ...
  Future<String?> _uploadPhoto({
    ...
    return ResortCloudinaryService
        .pickAndUploadImage(
  ...
  Future<void> _pickProfilePhoto() async {
    ...
    try {
      ...
      _message(
        'Resort logo/profile photo ready. '
        'Press Save Resort Profile.',
      );
    } catch (error) {
      _message(
  ...
  Future<void> _save() async {
    ...
    final DocumentReference<
            Map<String, dynamic>>?
        reference = _resortRef;

    if (user == null ||
        user.isAnonymous ||
        reference == null) {
      _message(
        'Resort Partner login is required.',
      );
      return;
    }
    ...
    try {
      ...
      setState(() {
        ...
        if (!_resortApproved) {
          _resortStatus = 'pending';
        }
      });

      _message(
        _resortApproved
            ? 'Resort profile saved successfully.'
            : 'Resort profile submitted. Admin approval is required before rooms can go live.',
      );
    } catch (error) {
      _message(
        'Could not save resort profile.\n'
        '$error',
      );
    } finally {
      if (mounted) {
                  ...
                  SizedBox(
                    ...
                    child:
                      ...
                      label: Text(
                        _exists
                            ? 'Save Resort Profile'
                            : 'Submit Resort for Admin Approval',
                        style:
                            const TextStyle(
                          fontWeight:
                              FontWeight
