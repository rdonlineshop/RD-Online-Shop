...
class AdminResortFeePage extends StatefulWidget {
  const AdminResortFeePage({super.key});

  @override
  State<AdminResortFeePage> createState() =>
      _AdminResortFeePageState();
}

class _AdminResortFeePageState
    extends State<AdminResortFeePage> {
  ...
  DocumentReference<Map<String, dynamic>>
      get _settingsRef =>
          FirebaseFirestore.instance
              .collection('resort_fee_settings')
              .doc('global');
  ...
  Future<void> _loadSettings() async {
    try {
      ...
      _message(
        'Could not load Resort fee settings.\n'
        '$error',
      );
    }
  }
  ...
  Future<void> _saveSettings() async {
    ...
    try {
      ...
      if (mounted) {
        _message(
          'Resort fee settings saved.',
        );
      }
    } catch (error) {
      _message(
        'Could not save Resort fee settings.\n'
        '$error',
      );
    } finally {
      if (mounted) {
  ...
  Future<void> _showGenerateInvoiceDialog() async {
    try {
      ...
      if (resortSnapshot.docs.isEmpty) {
        _message(
          'No approved active Resort found.',
        );
        return;
      }
      ...
      await showDialog<void>(
        ...
        builder:
          ...
          return _GenerateResortFeeInvoiceDialog(
            resorts: resortSnapshot.docs,
      ...
      _message(
        'Could not load Resorts for invoice generation.\n'
        '$error',
      );
    }
  }
  ...
  Future<String?> _generateInvoice({
    ...
    try {
      ...
      if (partnerId.isEmpty) {
        return 'Resort Partner ID is missing.';
      }
      ...
      await _audit(
        ...
        targetId: invoiceId,
        details:
            '${resort['name'] ?? 'Resort'} $billingMonth total $totalDue',
      );
                ...
                child: ListView(
                  ...
                  children: <Widget>[
                    ...
                    Container(
                      ...
                      child: const Text(
                        'Fee calculation uses paid Resort bookings created '
    ...
    return StreamBuilder<
      ...
      builder: (
        ...
        if (snapshot.hasError) {
          return Center(
            child: Text(
              'Could not load Resort fee invoices.\n'
              '${snapshot.error}',
              textAlign:
                  TextAlign.center,
            ),
    ...
    return StreamBuilder<
      ...
      builder: (
        ...
        if (snapshot.hasError) {
          return Center(
            child: Text(
              'Could not load Resort fee payments.\n'
              '${snapshot.error}',
              textAlign:
                  TextAlign.center,
            ),
  ...
  Widget _auditTab() {
    return StreamBuilder<
      ...
      stream: FirebaseFirestore.instance
          .collection('resort_fee_audit_logs')
          .snapshots(),
      builder: (
        BuildContext context,
        AsyncSnapshot<
        ...
        if (snapshot.hasError) {
          return Center(
            child: Text(
              'Could not load Resort fee audit log.\n'
              '${snapshot.error}',
              textAlign: TextAlign.center,
            ),
          );
          ...
          children: <Widget>[
            ...
            TextField(
              ...
              decoration:
                ...
                hintText:
                    'Bank / eWallet / office payment instructions for Resort Partners',
                border:
                    OutlineInputBorder(),
              ),
            ),
...
class _GenerateResortFeeInvoiceDialog
    extends StatefulWidget {
  const _GenerateResortFeeInvoiceDialog({
  ...
  @override
  State<_GenerateResortFeeInvoiceDialog>
      createState() =>
          _GenerateResortFeeInvoiceDialogState();
}

class _GenerateResortFeeInvoiceDialogState
    extends State<
        _GenerateResortFeeInvoiceDialog> {
  late String _resortId;
  late String _month;
          ...
          child: Column(
            children: <Widget>[
              ...
              const Text(
                ...
                'Resort bookings created in the selected month.',
                style: TextStyle(
                  fontSize: 12,
                  fontWeight:
                      FontWeight.w700,
