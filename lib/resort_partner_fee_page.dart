...
class ResortPartnerFeePage extends StatefulWidget {
  const ResortPartnerFeePage({super.key});

  @override
  State<ResortPartnerFeePage> createState() =>
      _ResortPartnerFeePageState();
}

class _ResortPartnerFeePageState
    extends State<ResortPartnerFeePage> {
  ...
  Future<void> _submitPayment(
    ...
    try {
      final QuerySnapshot<Map<String, dynamic>> existing =
          await FirebaseFirestore.instance
              .collection('resort_fee_payments')
              .where(
                'partnerId',
                isEqualTo: user.uid,
              )
    ...
    await showDialog<void>(
      ...
      builder: (BuildContext dialogContext) {
        return _ResortFeePaymentDialog(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: <Widget>[
            ...
            Text(
              data['resortName']?.toString() ?? 'Resort',
              style: const TextStyle(
        ...
        child: StreamBuilder<
          ...
          builder: (
            ...
            return StreamBuilder<
              ...
              stream: FirebaseFirestore.instance
                  .collection('resort_fee_invoices')
                  .where(
                    'partnerId',
                    isEqualTo: user.uid,
                  )
                            ...
                            if (invoices.isEmpty)
                              const Card(
                                child: Padding(
                                  ...
                                  child: Text(
                                    'No Resort fee invoice has been generated yet.',
                                    textAlign: TextAlign.center,
                                  ),
                                ),
                              )
...
class _ResortFeePaymentDialog extends StatefulWidget {
  const _ResortFeePaymentDialog({
  ...
  @override
  State<_ResortFeePaymentDialog> createState() =>
      _ResortFeePaymentDialogState();
}

class _ResortFeePaymentDialogState
    extends State<_ResortFeePaymentDialog> {
  ...
  Future<void> _uploadProof() async {
    ...
    try {
      final String? url =
          await ResortCloudinaryService.pickAndUploadImage(
        imageQuality: 82,
      );
    ...
    try {
      ...
      await paymentRef.set(
        <String, dynamic>{
          ...
          'partnerId': user.uid,
          'resortId':
              invoice['resortId']?.toString() ?? '',
          'resortName':
              invoice['resortName']?.toString() ?? 'Resort',
          'billingMonth':
