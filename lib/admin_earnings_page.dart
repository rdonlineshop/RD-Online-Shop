...
class _AdminEarningsPageState extends State<AdminEarningsPage> {
  ...
  Future<void> _exportPdf(List<_SettlementRow> rows) async {
    try {
      ...
      final String fileName =
          'RD_Earnings_${DateTime.now().millisecondsSinceEpoch}.pdf';
      await SharePlus.instance.share(
        ShareParams(
          ...
          text: 'RD Earnings & Commission Report',
        ),
      );
    } catch (e) {
      if (!mounted) return;
  ...
  Future<void> _exportExcel(List<_SettlementRow> rows) async {
    try {
      ...
      final String fileName =
          'RD_Earnings_${DateTime.now().millisecondsSinceEpoch}.xlsx';
      await SharePlus.instance.share(
        ShareParams(
          files: <XFile>[
            XFile.fromData(
          ...
          text: 'RD Earnings & Commission Excel Report',
        ),
      );
    } catch (e) {
      if (!mounted) return;
